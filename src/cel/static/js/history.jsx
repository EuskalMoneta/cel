...
var HistoryPage = React.createClass({
    ...
    componentDidMount() {
        var computeAccountList = (data) => {
            ...
            this.setState({currentAccount:res[0], accountList: res,
                           allAccount: data.result, currentSolde: data.result[0]}, this.refreshTable)
        }
    ...
    beginDateChange(date) {
        if (date <= this.state.endDate)
        {
            ...
            this.refs.container.error(
                __("Attention, la date de début doit être antérieure à celle de fin !"),
                "",
                {
                    timeOut: 5000,
    ...
    endDateChange(date) {
        if (date >= this.state.beginDate) {
            ...
            this.refs.container.error(
                __("Attention, la date de fin doit être postérieure à celle de début !"),
                "",
                {
                    timeOut: 5000,
                    extendedTimeOut: 10000,
    ...
    render() {
        ...
        if (this.state.currentSolde || this.state.currentSolde === 0) {
            var currentSoldeLabel = (
                <span className="solde-history-span">
                    {this.state.currentSolde.status.balance.replace('.', ',') + " " + this.state.currentSolde.currency.suffix}
                </span>
            )
        }
        else
        ...
        var debitFormatter = (cell, row) => {
            ...
            // we need to cast it in a Number object to use the toFixed method.
            if (cell < 0) {
                return Number(cell).toFixed(2).replace('.', ',')
            }
        }
        ...
        var historyTable = (
            <BootstrapTable
             ...
             options={options}>
                ...
                <TableHeaderColumn dataField="solde" dataFormat={soldeFormatter}>{__("Solde")}</TableHeaderColumn>
            </BootstrapTable>
        )
                ...
                <div className="col-md-10">
                    <HistoricalForm ref="historical-form">
                        <div className="form-group row" style={{marginBottom: 5}}>
                            <div className="col-sm-3 col-sm-offset-1" style={{paddingLeft: 25}}>{accountData}</div>
                        </div>
                        <div className="form-group row">
                            <div className="col-sm-3 col-sm-offset-1" style={{paddingLeft: 25}}>{balanceData}</div>
                        </div>
                        <div className="col-md-8 col-md-offset-1">
                            <div className="search-group" style={{borderRadius: 5, paddingBottom: 10}}>
                                ...
                                <div className="form-group row col-md-offset-1" style={{marginBottom: 0}}>
                                    <label
                                        className="control-label col-sm-1 col-sm-offset-1"
                                ...
                                <div className="form-group row col-md-offset-1" style={{marginBottom:0, marginTop:10}}>
                                    <label className="control-label col-sm-2 col-md-offset-5" 
                                           htmlFor="memberhistorical-date-start-end" 
                                           style={{paddingTop:10}}>
                                                Date début / fin
                                    </label>
                                    <div className="col-sm-2 memberhistorical"
                                         data-eusko="memberhistorical-date-start"
                                         style={{marginLeft: 10}}>
                                        <DatePicker
